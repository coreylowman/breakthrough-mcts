...
impl Node {
    fn memory_usage(&self) -> usize {
        std::mem::size_of::<Node>()
            + self.actions.capacity() * std::mem::size_of::<Action>()
            + self.children.capacity() * std::mem::size_of::<usize>()
            + self.unvisited_actions.capacity() * std::mem::size_of::<Action>()
    }
    fn new(id: usize, parent: Option<(&Node, &Action)>) -> Node {
        let (env, parent_id, my_action, is_over) = match parent {
            ...
            None => (BitBoardEnv::new(), 0, false, false),
        };
...
impl MCTS {
    fn memory_usage(&self) -> usize {
        ...
        mem += (self.nodes.capacity() - self.nodes.len()) * std::mem::size_of::<Node>();
        mem
    }
    ...
    fn with_capacity(id: usize, capacity: usize) -> MCTS {
        let mut root = Node::new(0, Option::None);
        root.my_action = id == WHITE;
...
fn codingame_main() {
    ...
    let mut mcts = MCTS::with_capacity(id, 10_000_000);
    ...
    loop {
        ...
        let (num_steps, millis) = mcts.explore_for(95);
        let action = mcts.best_action();
        let action_str = serialize_move(&action);
        println!(
            "{} {} in {}ms | {} / {}",
...
fn timed_first_explore() {
    ...
    let (num_steps, millis) = white_mcts.time_explore_n(100_000);
    eprintln!(
        "{} ({} in {}ms)... {} nodes",
        num_steps as f32 / millis as f32,
        num_steps,
...
fn run_game() {
    ...
    let mut white_mcts = MCTS::with_capacity(0, 2_500_000);
    println!("white {}ms", white_init_start.elapsed().as_millis());

    let black_init_start = Instant::now();
    let mut black_mcts = MCTS::with_capacity(1, 2_500_000);
    ...
    while !env.is_over() {
        ...
        let action = if env.player.id == WHITE {
            ...
            eprintln!(
                ...
                100.0
                    * (1.0
                        - ((post_node_size - white_mcts.root) as f32) / post_node_capacity as f32),
            );
            ...
            eprintln!(
                ...
                100.0
                    * (1.0
                        - ((post_node_size - black_mcts.root) as f32) / post_node_capacity as f32),
            );
...
// 1275mb (17.957747 mb/turn) 1270mb (17.887323 mb/turn) | 4068ms 3900ms
